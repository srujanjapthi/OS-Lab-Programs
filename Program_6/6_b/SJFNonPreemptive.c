...
void sjfScheduling(int n, Process processes[]) {
    int currTime = processes[0].arrivalTime, totCompleted = 0;
    int totWT = 0, totRT = 0, totTAT = 0;

    printf("\nGantt Chart:\n");
    while (totCompleted != n) {
        int currIdx = -1, minBurstTime = INT_MAX;
        for (int i = 0; i < n; i++) {
            Process *currProcess = (processes + i);
            if (currProcess->arrivalTime <= currTime) {
                if (!currProcess->isCompleted && currProcess->burstTime < minBurstTime) {
                    minBurstTime = currProcess->burstTime;
                    currIdx = i;
                }
            } else {
                break;
