/**
 * Problem Statement:
 * Implement SJF preemptive scheduling algorithm by defining process structure.
 */
...
void sjfSchedulingPreemptive(int n, Process processes[]) {
    int currTime = processes[0].arrivalTime, totCompleted = 0;
    int totWT = 0, totRT = 0, totTAT = 0;

    printf("\nGantt Chart:\n");
    while (totCompleted != n) {
        int currIdx = -1, minRT = INT_MAX;
        for (int i = 0; i < n; i++) {
            Process *currProcess = (processes + i);
            if (currProcess->arrivalTime <= currTime) {
                if (!currProcess->isCompleted && currProcess->remainingTime < minRT) {
                    minRT = currProcess->remainingTime;
                    currIdx = i;
                }
            } else {
                break;
        ...
        if (currIdx != -1) {
            ...
            if (--currProcess->remainingTime == 0) {
                ...
                printf("| P%d (%d) ", currProcess->pid, currTime);
                totCompleted++;
            }
        } else {
            currTime++;
...
void main() {
    ...
    sjfSchedulingPreemptive(n, processes);
